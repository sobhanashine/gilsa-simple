...
export default async function CompanySnapshot() {
  ...
  return (
    <section className="relative isolate overflow-hidden px-5 py-24">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_20%_30%,rgba(37,170,179,.18),transparent_30%),radial-gradient(circle_at_80%_18%,rgba(255,255,255,.07),transparent_24%)] [mask-image:linear-gradient(to_bottom,transparent,black_10%,black_90%,transparent)]" />
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:gap-0">
          <div className="relative z-10 w-full lg:w-[46%]">
            <div className="luxury-panel overflow-hidden rounded-[2.25rem] p-7 shadow-[0_0_80px_rgba(0,0,0,0.5)] md:p-10 lg:py-16">
              ...
              <h2 className="text-3xl font-black leading-tight tracking-[-.04em] sm:text-4xl md:text-5xl lg:text-6xl">{t('title')}</h2>
              <p className="mt-6 text-base leading-8 text-white/65 sm:text-lg sm:leading-9">{t('body')}</p>
          ...
          <div className="relative z-0 w-full space-y-5 lg:-ms-10 lg:w-[58%] lg:ps-16">
            <div className="grid gap-4 sm:grid-cols-2">
              {metricKeys.map((key, index) => {
                const Icon = metricIcons[index];
                return (
                  <article key={key} className="gold-card group overflow-hidden rounded-[1.75rem] p-6 shadow-lg transition hover:-translate-y-1 hover:border-[#25AAB3]/60">
                    ...
                    <p className="text-3xl font-black tracking-[-.04em] text-white sm:text-4xl md:text-5xl">{t(`metrics.${key}.value`)}</p>
                    <h3 className="mt-3 text-lg font-black tracking-[-.02em] sm:text-xl">{t(`metrics.${key}.title`)}</h3>
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#101014]/86 p-6 shadow-[0_24px_90px_rgba(0,0,0,.28)] md:p-8">
              ...
              <div className="relative mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-black tracking-[-.035em] sm:text-3xl md:text-4xl">{t('portfolio_title')}</h3>
                </div>
